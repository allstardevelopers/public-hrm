            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="emp_id" name="emp_id" required>
                                ...
                                @if (isset($emp_id))
                                <option value="{{encrypt(get_emp_id($employee->id))}}" {{ ($emp_id == get_emp_id($employee->id) ? 'selected' : '') }}>{{ $employee->name }}
                                </option>
                                @else
                                <option value="{{ encrypt(get_emp_id($employee->id)) }}">{{ $employee->name }}</option>
                                @endif
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-2 align-bottom">
                        ...
                        <button onclick="searchResult()" class="btn align-bottom btn-primary waves-effect waves-light">
                            Search
                        </button>
                    </div>
                </div>
...
<script>
    function searchResult() {
        ...
        if (emp_id != '', month_id != '') {
            let url = "{{ route('employee.attendance.team_report', ['id' => ':emp_id', 'm' => ':month_id', 'y' => ':year']) }}";
